...
void balanceMassMomentumHost(const int pn, const int* List,
    ...
    for (ip = 0; ip < pn; ip++) {
        ...
        for (il = 0; il < MAXN; il++) {
			...
			f = -(Pressure[ip] / powf((Density[ip] + 1.0f*hMatProp[Material[ip]][0]), 2) 
				+ Pressure[jp] / powf((Density[jp] + 1.0f*hMatProp[Material[jp]][0]), 2));
			...
			f *= 2.0f * Sound[ip];
			f /= Density[ip] + Density[jp] + 1.0f*hMatProp[Material[ip]][0] + 1.0f*hMatProp[Material[jp]][0];
            ...
            if (Material[ip] != Material[jp]) {
				f = 0.5f * w / w0 * Sound[ip] * Sound[jp] / iSmooth;
    ...
    if (ip < pn) {
        ...
        for (il = 0; il < MAXN; il++) {
            ...
            if (Material[ip] == Material[jp]) {
				...
				f = -(Pressure[ip] / powf((Density[ip] + 1.0f*dMatProp[Material[ip]][0]), 2) 
					+ Pressure[jp] / powf((Density[jp] + 1.0f*dMatProp[Material[jp]][0]), 2));
				f *= jMass * dwdr;
				...
				f *= 2.0f * Sound[ip];
				f /= Density[ip] + Density[jp] + 1.0f*dMatProp[Material[ip]][0] + 1.0f*dMatProp[Material[jp]][0];
				iDensityDot += jMass * f * dwdr;
			...
			// Calculate boundary repulsion
            if (Material[ip] != Material[jp]) {
				//f = 0.5f * w * Mass[jp] / Density[jp] / Smooth[jp] * powf(Sound[jp], 2);
...
void balanceEnergyHost(const int pn, const int* Material, 
    ...
    for (ip = 0; ip < pn; ip++) {
        ...
        iEnergyDot *= powf(iDensity + 1.0f*hMatProp[Material[ip]][0], -2.0f);
...
__global__ void balanceEnergyDevice(const int pn, const int* Material, 
    ...
    if (ip < pn) {
        ...
        iEnergyDot = iPressure * iDensityDot;
        iEnergyDot *= powf(iDensity + 1.0f*dMatProp[Material[ip]][0], -2.0f);
