...
c----------------------------------------------------------------------
c     Subroutine to read and store the FLUKA results from a binning 
c     binary file

c     trfluka-- translation of the fluka grid origin               [in]
...
c     ofluka-- the origin of the binning                          [out]
c     dfluka-- cell side of the binning                           [out]
c     nfluka-- number of cells for each index                     [out]
c     tfluka-- identifier for the fluka grid type                 [out]
c     datafluka-- data of the binning                             [out]
...
* Cartesian binning
	...
	subroutine fluka_heat(np, x, rho, dedt,
     &                      ofluka, dfluka, nfluka, tfluka, datafluka)


c----------------------------------------------------------------------
c     Subroutine to calculate the power deposition on the particles 
c     from the FLUKA results
	
c     np-- umber of particles                                      [in]
c     x-- particle position                                        [in]
c     rho-- density                                                [in]
c     dedt   : produced external heat, adding to energy Eq.       [out]
...
c     datafluka-- data of the binning                              [in]
      ...
      integer np
      double precision x(dim, maxn), rho(maxn), dedt(maxn),
     &                 ofluka(3), dfluka(3), datafluka(max_fluka)
	integer nfluka(3), tfluka
	...
	if (dim .eq. 3) then
     ...
     &                   * (1.0 + xr(3) * +1.0)
	        ...
	        dedt(ip) = (f1*v1 +f2*v2 +f3*v3 +f4*v4
...
c	        i = int((r -ofluka(1))/dfluka(1))
c	        j = int((phi -ofluka(2))/dfluka(2))
c	        k = int((x(3,ip) -ofluka(3))/dfluka(3))
c	        ic = i +j*nfluka(1) +k*nfluka(1)*nfluka(2) +1
	        
c	        dedt(ip) = datafluka(ic) / rho(ip)
	      endif
	    endif
        enddo
