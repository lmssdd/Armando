...
__global__ void kerInteractionOld(const struct grid dGrid,
...
int updateGridOld(void) {
    ...
    for (i = 0; i < hPN; i++) {
		...
		//printf("p %d %f %f %d \n",i, hPosX[i], hPosY[i], ix + iy * hGrid.nX);
    ...
    hSetStart[0] = 0;
    for (i = 1; i < hGrid.nX * hGrid.nY; i++) {
        hSetStart[i] = hSetStart[i -1] + hGrid.nump[i -1];
    }
    for (i = 0; i < hGrid.nX * hGrid.nY; i++) {
        hSetStop[i] = hSetStart[i +1];
    }
    for (i = 0; i < hPN; i++) {
        hIndex[i] = hGrid.perm[i];
    }
...
int updateListOld(void) {
	int ip, ic, ix, iy, il, i, j, jp, jt, jc, np;
	float dx, dy, dr;
...
int integrateRungeKutta3(void) {
    ...
    for (ts = 0; ts <= hRun.tsn; ts++) {
        ...
        // e permette una più efficace parallelizzazione!!!
...
int check() {
	...
	copyHostToDevice();
	kerHash <<< blocks, THREADS >>>
	(dGrid, dPosX, dPosY, dHash, dIndex);
	cudppSort(hSortHandle, dHash, dIndex, 24, hPN);
	sortArraysDevice();
	cutilSafeCall( cudaMemset(dSetStart, 0, dGrid.nX*dGrid.nY * sizeof(int)));
	cutilSafeCall( cudaMemset(dSetStop, 0, dGrid.nX*dGrid.nY * sizeof(int)));
	kerGrid <<< blocks, THREADS >>>
	(dSetStart, dSetStop, dHash);
	kerList <<< blocks, THREADS >>>
	(dList, dSetStart, dSetStop, dGrid, dPosX, dPosY);
	kerInteraction <<< blocks, THREADS >>>
	(dList, dPosX, dPosY, dVelX, dVelY, dDensity, dPressure, 
	dDensityDot, dVelDotX, dVelDotY);
	copyDeviceToHost();
