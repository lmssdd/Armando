...
__device__ __constant__ struct inlet dIn[10];
...
void inletHost(struct model *hm) {
    ...
    for (i = 0; i < 10; i++) if (hIn[i].Material != 0) {
		...
		if (hIn[i].Distance > hIn[i].Smooth / 1.2f) {
			...
			n = roundf(1.2f * hIn[i].R / hIn[i].Smooth);
...
int neighbourListDevice(struct model *dm) {
	...
	(dm->pn, dm->PosX, dm->PosY, dm->PosZ, dm->Hash, dGrid);
	...
	pout = thrust::count(tHash, tHash + dm->pn, dGrid.nX * dGrid.nY * dGrid.nZ);
	dm->pn -= pout;
	
	thrust::fill(tSetStart, tSetStart + dGrid.nX * dGrid.nY * dGrid.nZ, 0);
	thrust::fill(tSetStop, tSetStop + dGrid.nX * dGrid.nY * dGrid.nZ, 0);
