...
void initialize(thrust::device_vector<int>& v1, thrust::device_vector<int>& v2, thrust::device_vector<float>& v3)
{
  thrust::default_random_engine rng(123456);
  thrust::uniform_int_distribution<int> dist(10, 99);
  for(size_t i = 0; i < v1.size(); i++)
...
void print(thrust::device_vector<int>& v1, thrust::device_vector<int> v2, thrust::device_vector<float> v3)
{
  for(size_t i = 0; i < v1.size(); i++)
    std::cout << " (" << v1[i] << "," << std::setw(2) << v2[i] << "," << std::setw(3) << v3[i] << ")";
  std::cout << "\n";
}
...
int main(void)
{
  ...
  std::cout << "\nkey-value sorting\n";
  {
    ...
    initialize(keys, values, data);
    print(keys, values, data);
    thrust::sort_by_key(keys.begin(), keys.end(), 
                        thrust::make_zip_iterator(thrust::make_tuple(values.begin(), data.begin())));
    print(keys, values, data);
  }
